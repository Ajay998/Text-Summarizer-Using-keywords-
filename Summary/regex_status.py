...
labels = []
#keywords = ['HOSPITAL','Medical Center','Health Center','University','PLANO','Associates','Centers Of']
...
cursor.execute("select page_content from mt_page_file_contents where fk_case_id = 372 and page_number=30")
...
for i in sentence:
    ...
    for j in doc.ents:
        if j.label_=="ORG":
            ...
            labels.append(str(j))
print(labels)
cursor.execute('SELECT * FROM organisation_patterns')
patterns = cursor.fetchall()
res = [sub['patterns'] for sub in patterns]
print(res)
for i in res:
    for j in labels:
        if i in j:
            a = re.findall("(\w+ PLANO)", current_page_values)
            print(a)
